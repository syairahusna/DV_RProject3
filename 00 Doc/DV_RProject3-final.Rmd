...
```
It is worth mentioning that we purposely remove the labels on the x-axes for all qualitative variables. This is because there are a lot of distinct datapoints and they eventually made the axes become unreadable. In addition, company name and company id give us less interesting insights compared to the number of utilities they manage which could possibly be across many states.
...
Based on the plot above, it can be seen that Hawaii have significantly higher electricity rates for all types of rate compared to other states. It is also interesting to note here that the distribution between the three types of rate in within a state are quite similar for all states. 
...
Comparing the graph above with the previous graph on IOUs, it can be seen that non-IOUs provided electricity at much lower costs for all three types of rates, compared to IOUs, in almost all states, with the exception of Alaska. In fact, looking at Alaska and glancing at the scale of 'Value ($)' on the horizontal axis tell us what makes the distribution rate of non-IOUs quite skewed on the first violin plot above.

Note that state-by-state comparison is not applicable to states like Delaware which does not have non-IOUs and Armed Forces Pacific which do not have IOUs.
